...
public final class CollisionMatrix {
	...
	public boolean untraversable(int x, int y, EntityType entity, Direction direction) {
		...
		switch (direction) {
			case NORTH_WEST:
				return flagged(x, y, flags[southeast]);
			case NORTH:
				return flagged(x, y, flags[south]);
			case NORTH_EAST:
				return flagged(x, y, flags[southwest]);
			case EAST:
				return flagged(x, y, flags[west]);
			case SOUTH_EAST:
				return flagged(x, y, flags[northwest]);
			case SOUTH:
				return flagged(x, y, flags[north]);
			case SOUTH_WEST:
				return flagged(x, y, flags[northeast]);
			case WEST:
