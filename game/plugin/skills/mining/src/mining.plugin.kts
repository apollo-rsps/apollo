...
class MiningAction(val player: Player, val objectID: Int, val p: Position, val ore: Ore) : AsyncDistancedAction<Player>(PULSES, true, player, p, ORE_SIZE) {
     ...
     */
    private fun miningSuccessful(): Boolean {
        val level = mob.skillSet.getSkill(Skill.MINING).currentLevel
        val percent: Double
        if (ore.chanceOffset) {
            percent = (ore.chance * level + 1) * 100
        } else {
            percent = (ore.chance * level) * 100
        }
